...
# -- إدارة المظهر وسيناريوهات الألوان --
...
# --- أكواد CSS فائقة الدقة لإصلاح وضوح النصوص المزرقة بالكامل ---
st.markdown(f"""
    <style>
        ...
        /* 2. إصلاح النصوص المزرقة (تسميات الأدوات في الواجهة الرئيسية) */
        /* استهداف التسميات (Labels) فوق خانة رفع الملف وخانة كتابة وصف الصورة */
        .main div[data-testid="stWidgetLabel"] p {{
            color: #FFFFFF !important; /* لون أبيض ناصع ومضمون */
            font-size: 16px !important;
            font-weight: 600 !important;
            text-shadow: 1px 1px 2px rgba(0,0,0,0.5); /* إضافة ظل خفيف للوضوح */
        }}
        
        /* استهداف نص الوصف "200MB per file..." داخل مستطيل الرفع */
        .main div[data-testid="stFileUploadDropzone"] div p {{
            color: #E0E0E0 !important; /* لون رمادي فاتح جداً واضح */
            font-size: 14px !important;
        }}

        /* 3. تنسيقات إضافية للوضوح العام */
        ...
        /* 5. إخفاء القوائم وتنسيق الشات والمرفقات */
...
with st.sidebar:
    ...
    api_key_input = st.text_input("أدخل مفتاح Groq API Key:", type="password", key="groq_api_key_v3")

    st.divider()
    new_theme = st.selectbox("مظهر التطبيق:", ["داكن (Dark)", "فاتح (Light)"], index=0 if is_dark else 1, key="theme_selector_v3")
    ...
    selected_session = st.selectbox("اختر المحادثة:", session_names, index=session_names.index(st.session_state.current_session), key="session_selector_v3")
    
    if selected_session != st.session_state.current_session:
        st.session_state.current_session = selected_session
        st.rerun()

    if st.button("➕ محادثة جديدة", key="new_chat_btn_v3"):
    ...
    st.divider()
    if st.button("🗑️ مسح المحادثة الحالية", key="clear_chat_btn_v3"):
        st.session_state.sessions[st.session_state.current_session] = []
        st.rerun()

# --- وظائف مساعدة للشخصيات والمرفقات ---
persona_prompts = {
...
if api_key_input:
    ...
    try:
        ...
        # --- واجهة المرفقات وأدوات الصور الرئيسية ---
        st.subheader("📁 المرفقات وأدوات الصور:")
        col1, col2 = st.columns(2)
        with col1:
            uploaded_file = st.file_uploader("📷/📄 رفع صورة أو مستند (PDF/TXT):", type=["jpg", "jpeg", "png", "pdf", "txt"], key="doc_uploader_v3")
        with col2:
            gen_image_prompt = st.text_input("🖌️ توليد صورة جديدة (اكتب الوصف):", key="gen_image_prompt_v3")
            generate_btn = st.button("🎨 ارسم الصورة", key="gen_image_btn_v3")
    ...
    <div style="background-color: {chat_bg}; color: {text_color}; padding: 30px; border-radius: 15px; text-align: center; margin-top: 50px; border: 1px solid #454545;">
        ...
        <p style="font-size: 18px; margin-top: 15px; color: #E0E0E0 !important;">لبدء استخدام التطبيق وتحليل الملفات ورسم الصور بسرعات خارقة، يرجى إدخال <b>مفتاح Groq API Key</b> الخاص بك في الشريط الجانبي.</p>
    </div>
    """, unsafe_allow_html=True)
